# Hi, I’m Tejas S (@knightkid96)

I’m interested in cybersecurity and enjoy working on practical, real-world tech projects.  
I’ve completed the Google Cybersecurity Professional Certificate and I’m currently focusing on building skills in software engineering, QA automation, software testing, and system security.

---

## What I’m working on
- Strengthening my fundamentals in cybersecurity analysis  
- Building automation testing projects using Playwright  
- Improving problem-solving through hands-on development  

---

## Skills
- Cybersecurity: Threat analysis, vulnerability assessment  
- Testing: Manual testing, defect lifecycle, QA processes  
- Automation: Playwright (learning), Cypress (basics)  
- Database: SQL  
- Tools: Git, GitHub, Jira  
- Web: HTML, CSS, JavaScript  

---

## Projects
- **StegaCipher** – Secure communication using AES + image steganography  
- **QA Automation Framework** – Playwright-based automation framework (in progress)  
- **Car World Web Project** – Basic web app to showcase cars and features  

---

## Contact
- Email: knightkid96@gmail.com  
- LinkedIn: https://linkedin.com/in/tejas-shesh-mar002  
- Portfolio: https://theknightkid.wordpress.com/  
- YouTube: https://youtube.com/@knightkid.yt  
- Instagram: https://instagram.com/knightkid.yt  

---

I like breaking down complex problems and turning them into simple, practical solutions.
